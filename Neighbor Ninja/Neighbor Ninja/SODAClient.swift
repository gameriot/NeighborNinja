...
//        var request = URLRequest(url: URL(string: "https://\(self.domain)\(path).json?\(query)")!)
//        request.addValue("application/json", forHTTPHeaderField:"Accept")
//        request.addValue(self.token, forHTTPHeaderField:"X-App-Token")
//        
//        // Send it
//        let session = URLSession.shared
//        var task = session.dataTask(with: request, completionHandler: { data, response, reqError in
//            
